...
# 4. 绘图
# 字体策略：优先用宋体，如果没有就用黑体(微米黑)，最后用系统默认
font_family <- "SimSun" 
...
p <- ggplot(plot_data, aes(x = reorder(Display_Name, Rho), y = Rho)) +
  ...
  theme(
    ...
    plot.subtitle = element_text(size = 12, hjust = 0.5, color = "gray40", family = font_family),
    
    axis.text.y = element_text(size = 13, face = "bold", color = "black", family = font_family),
    axis.text.x = element_text(size = 12, color = "black", family = font_family),
    axis.title.x = element_text(size = 13, face = "bold", family = font_family),
    
    legend.position = "bottom",
    legend.text = element_text(family = font_family, size = 11),
    legend.title = element_text(face = "bold", family = font_family),
...
print(">>> 图片已生成。直接打开 3_Expert_Correlation_Lollipop_Final.svg 查看效果。")
```
...
# 1. 准备数据
if (!exists("analysis_data")) stop("数据未加载")
...
# 读取之前的映射表
name_map <- read_csv("2_Diff_Analysis_Sig_Candidates.csv", show_col_types = FALSE) %>%
  select(Computational_Name, Display_Name)

cor_table <- read_csv("3_Phase3_Correlation_All_Significant.csv", show_col_types = FALSE)
sig_vars <- cor_table %>% filter(P_Value < 0.05) %>% pull(Computational_Name)

# 2. 准备矩阵
heatmap_matrix <- analysis_data %>% select(Sample_ID, all_of(sig_vars)) %>% as.data.frame()
...
ann_col_df <- ann_col_df[, -1] # 去掉ID

# 对齐
common <- intersect(rownames(heatmap_matrix_scaled), rownames(ann_col_df))
heatmap_matrix_scaled <- heatmap_matrix_scaled[common, ]
ann_col_df <- ann_col_df[common, ]

# 4. 颜色定义 (注意名字要和上面中文列名对应)
# ==========================================================
# 仅修改颜色定义部分
# ==========================================================
ann_colors <- list(
...
# 保存SVG
ggsave("4_Cluster_Heatmap_Final_CN.svg", p_gg, 
       width = 10, height = 12, 
       device = svglite::svglite, 
       fix_text_size = FALSE)

print(">>> 绘图完成！请忽略警告，查看 4_Cluster_Heatmap_Final_CN.svg")
print(">>> 提示：图中大面积的蓝色是因为 Z-Score 标准化后，低含量样品较多，这属于正常现象。")
```
